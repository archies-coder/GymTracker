...
export default function RecordWorkoutScreen({ day }: { day: string }) {
  ...
  const [recordedSets, setRecordedSets] = useState<RecordedExercise[]>(
    getInitialSets(day)
  );

  function recordSet(
    exerciseName: string,
    set: { plates: number; reps: number }
  ) {
              ...
              <View>
                {inputs.map((currentInput) => (
                  <View key={currentInput.name} style={styles.inputContainer}>
                    ...
                    <Counter
                      count={currentInput[KEY_BY_UNIT[currentInput.unit]]}
